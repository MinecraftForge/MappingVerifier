...
project {
    ...
    params {
        text("git_main_branch", "maasters", label = "Git Main Branch", description = "The git main or default branch to use in VCS operations.", display = ParameterDisplay.HIDDEN, allowEmpty = false)
        text("github_repository_name", "MappingVerifier", label = "The github repository name. Used to connect to it in VCS Roots.", description = "This is the repository slug on github. So for example `MappingVerifier` or `MinecraftForge`. It is interpolated into the global VCS Roots.", display = ParameterDisplay.HIDDEN, allowEmpty = false)
        text("env.PUBLISHED_JAVA_ARTIFACT_ID", "mapping-verifier", label = "Published artifact id", description = "The maven coordinate artifact id that has been published by this build. Can not be empty.", allowEmpty = false)
    ...
    features {
        githubIssues {
            id = "MappingVerifier__IssueTracker"
            displayName = "MinecraftForge/MappingVerifier"
            repositoryURL = "https://github.com/MinecraftForge/MappingVerifier"
        }
    }
}
...
object BuildSecondaryBranches : BuildType({
    ...
    id("MappingVerifier__BuildSecondaryBranches")
...
object PullRequests : BuildType({
    ...
    id("MappingVerifier__PullRequests")
    name = "Pull Requests"
